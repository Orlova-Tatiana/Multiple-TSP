...
const {reverse, scrollTo} = require('../utils');
...
module.exports = class NextPrevByStepCrossoverStrategy {
    ...
    _next(parent1, parent2, tourManager) {
        ...
        const start = path1[0];
        let child = this._crossover(path1, path2, tourManager);
        child = scrollTo(child, start);
    ...
    _prev(parent1, parent2, tourManager) {
        ...
        let child = this._crossover(path1, path2, tourManager);
        child = scrollTo(child, start);
        child = reverse(child, 0, this._step);
    ...
    _crossover(path1, path2, tourManager) {
        ...
        let c = path1[randomInt(this._step + 1, path1.length - 1)];
        let childI = 0;
        child[childI++] = c;
